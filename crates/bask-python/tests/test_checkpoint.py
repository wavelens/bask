...
def test_source_prune_and_process_later():
    ...
    def build(consumer):
        ...
        engine.register(Feed, _reader(reads))
        engine.register(Row, lambda row, ctx: ctx.emit(Saved(row.id)))
        if consumer:
            engine.register(Saved, lambda saved, ctx: consumed.append(saved.id))
...
def test_key_only_side_effect_runs_once():
    ...
    def build():
        engine = Engine(concurrency=1, store=store)
        engine.register(Ping, lambda ping, ctx: sends.append(ping.id))
        engine.seed(Ping(7))
        return engine
